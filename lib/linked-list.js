...
class LinkedList {
    ...
    */
    ...
    Time Complexity: O(n)
    Space Complexity: O(n) maybe squared?
    */
    findNodeWhereNext(head, nextValue) {
        current = head;
        while (!!current && !!current.next && current.next.value !== nextValue) {
            current = current.next;
        }
    ...
    reverse() {
        ...
        this.#head = findNodeWhereNext(current, null); //point the head to the last node

        let next = this.#head;
        let prev = null;
        while (prev !== oldHead) {
            let prev = findNodeWhereNext(oldHead, next)
            next.next = prev;
            next = prev;
        }
